...
ChatGPT 分享对话里常有一边生成一边重画的情况，中间草稿不该进第二大脑。架构为 **「AI 语义分类 + 程序确定性执行」**——AI 只负责理解语义，去留由程序按规则算出来（可复现、不漂移）：

1. **抓取与描述**：无头浏览器抓取全部图片（自动压缩为 JPEG），豆包视觉模型逐张写内容描述
2. **AI 语义分类**：主模型对「每条用户反馈」分类（approve / extend / modify / negate / uncertain）+ 分配修改链（lineageId），只输出 图片标识 + 用户原话证据 + 置信度，**不直接输出去留**
3. **程序确定性执行**（`lib/judge.js`，纯逻辑、16 项单元测试覆盖）按四条规则计算：
   - 明确认可 / 基于它延伸 → 保留
   - 明确否定（高置信 + 原文强否定证据）→ 即使无替代也排除
   - 修改 + 存在**同一修改链**的替代图 → 排除旧图（新口味/延伸/其他任务不算替代）
   - 修改但无同链替代 / 无法确定 / 低置信（<0.8）/ 证据非原文 → 默认保留（宁可多留）
   - 明确认可过的版本锁定保留（优先于其它规则）
4. **只保存保留的图**（废图不落盘）；分类结果按「对话哈希 + 图片 hex」**缓存**到本机，重复导入同一链接结果稳定
5. **兜底**：模型不可用时保守保留全部（避免误删）；关键词规则仅兜底处理「不要这张 / 这版不行 / 重画这张」等强否定
6. **不抓的图**：用户上传的参考图默认排除；ChatGPT 分享页未渲染的图无法抓取（平台限制）
...
│       └── judge.js              # 图片去留的确定性规则引擎（纯逻辑，不调模型）
...
├── tests/
│   └── apply-rules.test.mjs      # 规则引擎离线单元测试（16 项，不联网）
...
- **测试**：规则引擎离线单测 `node tests/apply-rules.test.mjs`（不联网、纯逻辑）；真实对话/图片夹具放在 git 忽略的 `tests-local/`（隐私），不入仓库
- **图片分类缓存**：本机 `~/.dsh/plugins/second-brain/judge-cache/`（不入仓库），删除可强制重新分类
