...
ChatGPT 分享对话里常有一边生成一边重画的情况，中间草稿不该进第二大脑。插件**两段式 AI 判断**后只保存保留的图（废图不落盘）：

1. **看图**：无头浏览器抓取全部图片（自动压缩为 JPEG），豆包视觉模型逐张写内容描述
2. **判断**：主模型结合「图片描述 + 对话上下文（用户此前要求/生成后助手说明/用户随后反馈）」按以下**四条规则**决定去留：
   - **明确认可、继续引用、或基于它扩展**的图片（如「这个好」「就用这个」「根据这个延伸其他口味」「再做一个草莓味的」）→ 保留
   - **明确否定、要求重做，并且后面出现了替代版本** → 排除旧图
   - **同一设计的连续修改版本**：只保留最终版本；但用户**明确认可过**的中间基础版本也保留
   - **无法确定** → 默认保留，避免误删（宁可多留，不可误删）
3. **兜底**：模型不可用（未配置 / 调用失败 / 输出无法解析）时，保守保留全部图片，避免误删
4. **不抓的图**：用户自己上传的参考图（role=user 带图）不算 AI 生成结果，默认排除；ChatGPT 分享页未渲染的图也无法抓取（属平台限制）
